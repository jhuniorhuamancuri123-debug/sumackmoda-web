...
export default function CheckoutPage() {
  ...
  function construirMensajeWhatsApp() {
    ...
    mensaje += '📦 *MI PEDIDO:*\n';
    items.forEach(item => {
      mensaje += `• ${item.nombre} / ${item.color} / Talla ${item.talla} x${item.cantidad} = S/. ${(item.precio * item.cantidad).toFixed(2)}\n`;
    });
    mensaje += `\n💰 *Total: S/. ${Number(total).toFixed(2)}*\n`;
    mensaje += `\n🚚 *Envío preferido:* ${envioSeleccionado.label}`;
    if (envioSeleccionado.recargo === 0) {
      mensaje += ' (sin recargo)';
    } else {
      mensaje += ' (coordinar costo)';
    }
  ...
  function handleWhatsApp() {
    ...
    window.open(`https://wa.me/51968267313?text=${mensaje}`, '_blank');
  }

  if (items.length === 0) return (
    <div style={{
      paddingTop: '120px', minHeight: '60vh',
      ...
      <a href="/catalogo" style={{
        ...
        color: 'var(--marron)', textDecoration: 'none',
        borderBottom: '1px solid var(--marron)', paddingBottom: '2px',
      }}>
        Ver catálogo
      </a>
    </div>
    ...
    <div style={{ paddingTop: '80px', minHeight: '100vh' }}>
      <div style={{
            ...
            <div key={item.itemId} style={{
              ...
              <div style={{ display: 'flex', gap: '1rem', alignItems: 'center' }}>
                <div style={{
                  ...
                  background: item.hexColor || 'var(--crema)',
                  overflow: 'hidden',
                }}>
                  {item.imagen && (
              ...
              <p style={{ fontWeight: 600, color: 'var(--marron)', fontSize: '0.95rem' }}>
        ...
        <div style={{
          ...
          <span style={{ fontFamily: 'var(--font-body)', fontWeight: 600, fontSize: '1rem' }}>
            Total
          </span>
          <span style={{
            fontFamily: 'var(--font-body)', fontWeight: 700,
            fontSize: '1.3rem', color: 'var(--marron)',
          }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
            {envios.map(e => (
              <button key={e.id} onClick={() => setEnvio(e.id)}
                style={{
                  padding: '0.9rem 1rem',
                  border: envio === e.id ? '2px solid var(--negro)' : '1px solid var(--gris-claro)',
                  background: envio === e.id ? 'var(--negro)' : 'transparent',
                  color: envio === e.id ? 'var(--blanco)' : 'var(--negro)',
                ...
                <span style={{
                  fontSize: '0.78rem',
                  color: envio === e.id ? 'rgba(255,255,255,0.7)' : 'var(--gris)',
                }}>
                  {e.detalle}
                </span>
              </button>
        ...
        <div style={{
          ...
          <p style={{ fontSize: '0.88rem', lineHeight: 1.6, color: '#166534' }}>
            <strong>¿Cómo funciona?</strong> Al hacer click en el botón, se abrirá WhatsApp con
            tu pedido completo ya redactado — productos, tallas, colores y total.
            Solo envía el mensaje y te atendemos al instante.
            <strong> Es rápido, sin formularios y sin complicaciones.</strong>
          </p>
        </div>
        ...
        <p style={{
          fontFamily: 'var(--font-body)', fontSize: '0.75rem',
          color: 'var(--gris)', textAlign: 'center', marginTop: '1rem',
        }}>
