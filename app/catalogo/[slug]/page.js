...
export default function ProductoPage({ params }) {
  ...
  async function cargarProducto() {
    const { data } = await supabase
      .from('productos')
      .select('id, cod_modelo, cod_color, cod_talla, nombre, precio, stock, colores (cod_color, nombre_color, hex_color)')
  ...
  async function handleAgregar() {
    ...
    agregar({
      ...
      stockMax: productoSeleccionado.stock,
  ...
  if (loading) return (
    <div style={{paddingTop:'120px',textAlign:'center',fontFamily:'var(--font-body)',fontSize:'1rem',color:'var(--gris)'}}>
      Cargando producto...
    </div>
  );
  ...
  if (noEncontrado) return (
    <div style={{paddingTop:'120px',textAlign:'center',fontFamily:'var(--font-body)'}}>
      ...
      <a href="/catalogo" style={{color:'var(--marron)',textDecoration:'none',borderBottom:'1px solid var(--marron)'}}>
        Volver al catalogo
      </a>
    </div>
  );
      ...
      <div className="producto-inner">
        <div className="producto-galeria">
          <div className="producto-foto-principal">
            {!imgError && imgUrl ? (
              <img src={imgUrl} alt={nombreModelo} onError={() => setImgError(true)}
                style={{width:'100%',height:'100%',objectFit:'cover'}} />
            ) : (
              <div style={{width:'100%',height:'100%',background:colores.find(c=>c.cod_color===colorSeleccionado)?.hex_color||'#e8e4dc',display:'flex',alignItems:'center',justifyContent:'center',fontFamily:'var(--font-display)',fontSize:'2rem',fontWeight:900,color:'rgba(255,255,255,0.4)',letterSpacing:'0.2em'}}>
                SUMACK
              </div>
            )}
          </div>
        ...
        <div className="producto-info">
          ...
          <button className="btn-agregar"
            ...
            {agregando ? 'Agregado ✓' : !colorSeleccionado || !tallaSeleccionada ? 'Selecciona color y talla' : 'Agregar al carrito'}
          </button>

          <a href={`https://wa.me/51968267313?text=Hola, me interesa el producto ${nombreModelo}`}
            target="_blank" rel="noopener"
            style={{display:'flex',alignItems:'center',gap:'0.5rem',fontFamily:'var(--font-body)',fontSize:'0.85rem',letterSpacing:'0.1em',color:'var(--gris)',textDecoration:'none',justifyContent:'center',padding:'0.75rem',border:'1px solid var(--gris-claro)',transition:'all 0.2s',marginTop:'0.5rem'}}
